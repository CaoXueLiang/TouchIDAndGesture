...
-(void)drawRect:(CGRect)rect{
    ...
    //设置上下文属性
    //1.设置线条颜色
    //if (CurrentUser.showGesturePath)
    //{
...
-(void)lockViewPrepare{
    self.marginValue = 45 * [UIScreen mainScreen].bounds.size.width / 375.0;
...
-(void)layoutSubviews{
    ...
    CGFloat itemViewWH = (self.frame.size.width - 4 * self.marginValue) /3.0f;
    ...
    [self.subviews enumerateObjectsUsingBlock:^(UIView *subview, NSUInteger idx, BOOL *stop) {
        ...
        CGFloat x = self.marginValue * (row +1) + row * itemViewWH;
        
        CGFloat y = self.marginValue * (col +1) + col * itemViewWH;
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    }else if(CoreLockTypeVeryfiPwd == _type || CoreLockTypeScreenLoaker == _type){//验证密码
...
-(void)setpwdCheck{
    ...
    }else if (CoreLockTypeScreenLoaker == _type){//屏幕解锁
        if(_screenLoakerBlock != nil) _screenLoakerBlock(self.pwdM);
    }
    
}

-(void)setIsOldPwdVerifySuccess:(BOOL)isOldPwdVerifySuccess
{
    _isOldPwdVerifySuccess = isOldPwdVerifySuccess;
    if(!_modify_VeriryOldRight){
        
        if(_verifyPwdBlock != nil){
            
            _modify_VeriryOldRight = isOldPwdVerifySuccess;
        }
...
-(void)lockHandle:(NSSet *)touches{
    ...
    //记录密码
    [self.pwdM appendString:[NSString stringWithFormat:@"%ld%ld",itemView.tag/3,itemView.tag%3]];
